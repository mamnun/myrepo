...
#            output.write(finalArray)
    ...
    def keep_sending_video(self,dest_stream, segmentToStart=None, totalSegmentToSend=0):
        try:
            ...
            traceback.print_exc()
        try:
            #self.AKAMaiQueue.join();
            akhds.cleanup()                                    
        except:pass
        self.status='finished'


    def akamaithread(self,dest_stream):
        while True:
            #print 'threading here'
            item = self.AKAMaiQueue.get()
            #print 'threading got item', len(item)
            if self.g_stopEvent.isSet():
                return
            frameHeader,data,framePad,keyData =item
            data=akhds.tagDecrypt(data,keyData)
            seglen1=self.addBytesToOutput(frameHeader,data,framePad,1,dest_stream)
            #print 'threading sending data ', seglen1
            self.AKAMaiQueue.task_done()
...
#                                                    akhds.init()
                                                    #data=akhds.tagDecrypt(data,keyData)
                                                    #print 'adding queue'
                                                    self.AKAMaiQueue.put([frameHeader,data,framePad,keyData ])
                                                    if self.AkamaiThread==None:
                                                        #print 'creating thread'
                                                        self.AkamaiThread = Thread(target=self.akamaithread,args=(dest_stream,))
                                                        self.AkamaiThread.daemon = True
                                                        self.AkamaiThread.start()
